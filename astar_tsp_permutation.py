...
class Maze:
    ...
    def run(self):
        ...
        for ie, ends in enumerate(permutations(self.coins)):
            ax.clear()
            ...
            for p, p_type in self.a_star(list(ends)):
        ...
        plt.title(str(mc))
        for i, point in enumerate(mp[:-1]):
            ax.plot([point[1], mp[i+1][1]], [point[0], mp[i+1][0]], color="green")
            plt.pause(0.7)
        plt.show()

    def a_star(self, ends):
        tc = 0
        self.board = copy.deepcopy(self.b)# self.b.copy()
        #ends = self.coins.copy()
        start = self.start
        cost = lambda a, b: distance(a, b) + self.board[b[0]][b[1]]
        heuristic = lambda p: distance(p, ends[0]) #min([distance(p, e) for e in ends])
        ...
        while unchecked_points and ends:
            ...
            if current_point == ends[0]:#in ends:
                e = current_point
                ends.remove(e)
                p = []
                c = 0
